...
numBins=ceil(numDataPoints/dataPointsPerBin);

PSTH=zeros(numChannels,numBins);

% take care that signal length is an integer multiple of bin size
%  by adding zeros to the last bin's values
useableDataLength=numBins*dataPointsPerBin;
diff=useableDataLength-numDataPoints;
inputData=horzcat(inputData,zeros(numChannels,diff));
