...
if showMapOptions.showModelOutput
    plotInstructions.figureNo=89;     

    % plot signal (1)
    plotInstructions.displaydt=dt;
    plotInstructions.numPlots=3;
    
    %single BM plot hack RJames
%     plotInstructions.numPlots=1;
%     plotInstructions.subPlotNo=1;
%     plotInstructions.yValues= savedBFlist;
%     [r c]=size(DRNLoutput);
%     if r>1
%         plotInstructions.title= ['BM displacement from chirp input (60dBSPL) - Meddis Model (gammatone filterbank)'];        
%         UTIL_plotMatrix(abs(DRNLoutput), plotInstructions);
%         xlabel('Time (s)')
%         ylabel('BM Center Frequency (Hz)');
%     end
...
%     end
  ...
  %          UTIL_plotMatrix_Rob(ANoutput, plotInstructions);
            ...
            %UTIL_plotMatrix_Rob(CNoutput, plotInstructions);
